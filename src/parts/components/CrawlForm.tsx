...
const CrawlForm: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  className,
}) => {
  const [url, setUrl] = React.useState('');
  const [error, setError] = React.useState('');
  const { makeRequest: makeCrawlRequest, loading: isLoading } =
    useApiRequest<Response>({
      endpoint: '/crawl',
      method: 'POST',
      requiresAuth: true,
      manual: true,
    ...
    if (response.error) {
      setError(
        response.error.message ||
          'An error occurred while adding the URL, try again later or try a different URL.',
      );
      return;
    }
  ...
  return (
    <div
      className={
        'flex flex-col gap-4 w-full' + (className ? ` ${className}` : '')
      }
    >
      <form
